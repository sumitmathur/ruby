...
if mode == "disable"
  ...
  if code  == 201
      puts "Pagerduty :: Application #{app_name} is maintenance mode now"   
    else
      puts "Pagerduty :: Application #{app_name} is failed to put in maintenance mode"
   ...
   code = delete_maintenance_windows(service_ids,subdomain,token)
    if code  == 204
      puts "Pagerduty :: Application #{app_name} is active now"   
    else
      puts "Pagerduty :: Application #{app_name} is failed to take out of maintenance"
...
if ARGV.length != 4 
 abort("USAGE :: silencer.rb application_name mode dc time") 
end
